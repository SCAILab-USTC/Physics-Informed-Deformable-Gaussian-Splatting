...
def compute_color(u, v):
    ...
    [h, w] = u.shape
    img = np.zeros([h, w, 3])
    nanIdx = nanIdx = np.isnan(u) | np.isnan(v) | np.isinf(u) | np.isinf(v)
    u[nanIdx] = 0
    v[nanIdx] = 0
...
def flow_to_image(flow):
    ...
    :param flow: optical flow map, shape [H,W,2]
    ...
    maxu = np.maximum(-999.0, np.max(u))
    minu = np.minimum( 999.0, np.min(u))
    maxv = np.maximum(-999.0, np.max(v))
    minv = np.minimum( 999.0, np.min(v))

    rad    = np.sqrt(u**2 + v**2)
    maxrad = np.maximum(-1.0, np.max(rad))
